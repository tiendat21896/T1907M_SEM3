...
namespace WeatherAppDemo
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private void Page_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                var postion = await LocationData.getProsition();
                var lat = postion.Coordinate.Latitude;
                var lon = postion.Coordinate.Latitude;

                RootObject myWeather = await APIManager.GetWeather(lat, lon);

                string icon = String.Format("ms-app:///Assets/Weather/{0}.png", myWeather.weather[0].icon);

                ResultImage.Source = new BitmapImage(new Uri(icon, UriKind.Absolute));
                TempTextBlock.Text = ((double)myWeather.main.temp).ToString();
                DescriptipnTextBlock.Text = myWeather.weather[0].description;
